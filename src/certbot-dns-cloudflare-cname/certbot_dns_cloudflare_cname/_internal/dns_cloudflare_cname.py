...
class Authenticator(dns_common.DNSAuthenticator):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.resolved_validation_name = None
    ...
    def _perform(self, domain: str, validation_name: str, validation: str) -> None:
        self.resolved_domain = domain
        self.resolved_validation_name = validation_name

        if self.conf('follow-cnames') == 'true':
            notify('CNAME aliasing enabled')
            self.resolved_domain, self.resolved_validation_name = self._resolve_cname_aliasing(domain, validation_name)

        self._get_cloudflare_client().add_txt_record(self.resolved_domain, self.resolved_validation_name, validation, self.ttl)

    def _cleanup(self, domain: str, validation_name: str, validation: str) -> None:
        self._get_cloudflare_client().del_txt_record(self.resolved_domain, self.resolved_validation_name, validation)
    ...
    def _resolve_cname_aliasing(self, domain: str, validation_name: str) -> str:
        """
        Recursively follow validation_name CNAME.

        :param str domain: The domain.
        :param str validation_name: ACME DNS validation name.
        ...
        try:
            ...
            logger.error('Could not import dnspython module.')
            return domain, validation_name

        resolver = dns.resolver.Resolver()
        resolved_name = dns.name.from_text(validation_name)

        while True:
            notify('Resolving {resolved_name}'.format(resolved_name=resolved_name))
            try:
                dns_resp = resolver.query(resolved_name, dns.rdatatype.CNAME)

                if len(dns_resp) == 1 and dns_resp.rdtype == dns.rdatatype.CNAME:
                    resolved_name = dns_resp[0].target
                    notify('Resolved to CNAME {resolved_name}'.format(resolved_name=resolved_name))
                    continue
                elif len(dns_resp) == 1 and dns_resp.rdtype == dns.rdatatype.TXT:
                    resolved_name = dns_resp[0].target
                    notify('Resolved to TXT {resolved_name}'.format(resolved_name=resolved_name))
                    break
                else:
                    break
            except:
        ...
        resolved_name = resolved_name.to_text(omit_final_dot=True)
        new_domain = '.'.join(resolved_name.split('.')[-2:])
        return new_domain, resolved_name
